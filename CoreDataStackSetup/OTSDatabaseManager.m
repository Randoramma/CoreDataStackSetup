...
static NSString * const kOTSDataBaseManagerErrorDomain = @"OTSErrorDomain";
...
- (void)setupCoreDataStackWithCompletionHandler:(OTSDatabaseManagerCompletionHandler)handler
{
	...
	dispatch_async(queue, ^{
		...
		if (![[NSFileManager defaultManager] createDirectoryAtURL:storeURL withIntermediateDirectories:YES attributes:nil error:&error]) {
			...
			if (error) {
				customError = [NSError errorWithDomain:kOTSDataBaseManagerErrorDomain code:error.code userInfo:error.userInfo];
			}
...
- (NSError *)createErrorWithCode:(NSUInteger)code desc:(NSString *)description suggestion:(NSString *)suggestion options:(NSArray *)options
{
	...
	NSError *error = [NSError errorWithDomain:kOTSDataBaseManagerErrorDomain code:code userInfo:userInfo];
